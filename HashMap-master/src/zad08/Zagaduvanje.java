...
public class Zagaduvanje {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            ...
            int index = table.search(s[0]);

            if (index == -1) {
                table.insert(s[0], s[1]);
            } else {
                table.insert(s[0], s[1] + " " + table.buckets[index].value);
            }
        }

        String opstina = br.readLine().trim();
        int index = table.search(opstina);

        String[] pm = table.buckets[index].value.split(" ");
        float average = 0;

        for (String s : pm) {
            average += Float.parseFloat(s);
        }
