...
class DLLSpoi {
    public static void spoi(DLL<Integer> list, int m) {
        for (int i = 0; i < m; i++) {
            int sum = 0;
            DLLNode<Integer> iteratorFirst = list.getFirst();
            DLLNode<Integer> last = list.getLast();

            DLLNode<Integer> tmp = iteratorFirst;
            while (iteratorFirst != null) {
                sum += iteratorFirst.element;
                if (sum <= last.element) {
                    tmp = tmp.succ;
                } else {
                ...
                iteratorFirst = iteratorFirst.succ;
            }
        }
    }
